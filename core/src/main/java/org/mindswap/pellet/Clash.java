...
public class Clash {
	...
	private Clash(Node node, ClashType type, TimeDS depends) {
		this.setDepends( depends );
		this.setNode( node );
		this.setType( type );
	}

	private Clash(Node node, ClashType type, TimeDS depends, ATerm[] args) {
		this.setDepends( depends );
		this.setNode( node );
		this.setType( type );
		this.args = args;
	...
	private Clash(Node node, ClashType type, TimeDS depends, String explanation) {
		this.setDepends( depends );
		this.setNode( node );
		this.setType( type );
		this.explanation = explanation;
	...
	public static Clash unexplained(Node node, TimeDS depends) {
		return new Clash(node, ClashType.UNEXPLAINED, depends);
	}

	public static Clash unexplained(Node node, TimeDS depends, String msg) {
		return new Clash(node, ClashType.UNEXPLAINED, depends, msg);
	}

	public static Clash atomic(Node node, TimeDS depends) {
		return new Clash(node, ClashType.ATOMIC, depends);
	}

	public static Clash atomic(Node node, TimeDS depends, ATermAppl c) {
		return new Clash(node, ClashType.ATOMIC, depends, new ATerm[] {c});
	}

	public static Clash bottomProperty(Node node, TimeDS depends, ATermAppl p) {
		return new Clash(node, ClashType.BOTTOM_PROP, depends, new ATerm[] {p});
	}

	public static Clash maxCardinality(Node node, TimeDS depends) {
		return new Clash(node, ClashType.MAX_CARD, depends);
	}

	public static Clash maxCardinality(Node node, TimeDS depends, ATermAppl r, int n) {
		return new Clash(node, ClashType.MAX_CARD, depends, new ATerm[] {r, ATermUtils.getFactory().makeInt(n)});
	}

	public static Clash minMax(Node node, TimeDS depends) {
		return new Clash(node, ClashType.MIN_MAX, depends);
	}

	public static Clash functionalCardinality(Node node, TimeDS depends) {
		return new Clash(node, ClashType.FUNC_MAX_CARD, depends);
	}

	public static Clash functionalCardinality(Node node, TimeDS depends, ATermAppl r) {
		return new Clash(node, ClashType.FUNC_MAX_CARD, depends, new ATerm[] {r});
	}

	public static Clash missingDatatype(Node node, TimeDS depends) {
		return new Clash(node, ClashType.MISSING_DATATYPE, depends);
	}

	public static Clash missingDatatype(Node node, TimeDS depends, ATermAppl value, ATermAppl datatype) {
		return new Clash(node, ClashType.MISSING_DATATYPE, depends, new ATermAppl[] {value, datatype});
	}

	public static Clash nominal(Node node, TimeDS depends) {
		assert depends.size() == 1 && depends.time().isUniversal();
		return new Clash(node, ClashType.NOMINAL, depends);
	}

	public static Clash nominal(Node node, TimeDS depends, ATermAppl other) {
		return new Clash(node, ClashType.NOMINAL, depends, new ATermAppl[] {other});
	}

	public static Clash valueDatatype(Node node, TimeDS depends) {
		return new Clash(node, ClashType.VALUE_DATATYPE, depends);
	}

	public static Clash valueDatatype(Node node, TimeDS depends, ATermAppl value, ATermAppl datatype) {
		return new Clash(node, ClashType.VALUE_DATATYPE, depends, new ATermAppl[] {value, datatype});
	}

	public static Clash emptyDatatype(Node node, TimeDS depends) {
		return new Clash(node, ClashType.EMPTY_DATATYPE, depends);
	}

	public static Clash emptyDatatype(Node node, TimeDS depends, ATermAppl[] datatypes) {
		return new Clash(node, ClashType.EMPTY_DATATYPE, depends, datatypes);
	}

	public static Clash invalidLiteral(Node node, TimeDS depends) {
		return new Clash(node, ClashType.INVALID_LITERAL, depends);
	}

	public static Clash invalidLiteral(Node node, TimeDS depends, ATermAppl value) {
		return new Clash(node, ClashType.INVALID_LITERAL, depends, new ATermAppl[] {value});
	}

	public static Clash disjointProps(Node node, TimeDS depends) {
		return new Clash(node, ClashType.DISJOINT_PROPS, depends);
	}

	public static Clash disjointProps(Node node, TimeDS depends, ATermAppl p1, ATermAppl p2) {
	 ...
	 */
	public void setDepends(TimeDS depends) {
